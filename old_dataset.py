    ...
    def is_valid(imgpath):
        ...
        if os.path.getsize(labpath):
            ...
            if bs is not None:
                bs = np.reshape(bs, (-1, 5))
                clsset = set(bs[:, 0].astype(np.int).tolist())
                if not clsset.isdisjoint(set(cfg.base_ids)):  # whether current image contains classes of base classes
                    return True
        return False
...
class MetaDataset(Dataset):
    def __init__(self,
        ...
        # do not know why using 500 * 64 * factor, also doo not know what does factor means
