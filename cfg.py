...
# Maximum number of bboxes per category? i think it's per image.
__C.max_boxes = 50
...
# do not use multiscale
__C.multiscale = False
...
def __configure_data(dataopt):
    ...
    if 'meta_in_type' in dataopt:
        __C.meta_in_type = int(dataopt['meta_in_type'])
    ...
    print('novel_classes:', __C.novel_classes)
    ...
    print('base_classes:', __C.base_classes)
    ...
    if not __C.multiscale:
        __C.backup += 'fix'
    if __C.meta_in_type != 2:  # metain_type?
        __C.backup = add_backup(__C.backup, 'in{}'.format(__C.meta_in_type))
...
def __configure_meta(metaopt):
    ...
    # meta input type
    if factor == 0:
        if __C.meta_in_type == 1:
            metaopt['channels'] = 3
        elif __C.meta_in_type == 2:
            metaopt['channels'] = 4
        elif __C.meta_in_type == 3:
            metaopt['channels'] = 7
        elif __C.meta_in_type == 4:
            metaopt['channels'] = 6
        else:
            raise NotImplementedError('Meta input type not found: {}'.format(__C.meta_in_type))
    elif factor == 4:
        if __C.meta_in_type == 1:
            metaopt['channels'] = 64
        elif __C.meta_in_type == 2:
    ...
    for block in blocks:
        ...
        if block['type'] == 'net' or block['type'] == 'learnet':
            ...
            print('%5d %-6s %4d  %d x %d / %d   %3d x %3d x%4d   ->   %3d x %3d x%4d' % (
                ind, name, filters, kernel_size, kernel_size, stride, prev_width, prev_height, prev_filters, width,
                height,
                filters))
            prev_width = width
            prev_height = height
            ...
            print('%5d %-6s       %d x %d / %d   %3d x %3d x%4d   ->   %3d x %3d x%4d' % (
                ind, 'glomax', pool_size, pool_size, stride, prev_width, prev_height, prev_filters, width, height,
                filters))
            prev_width = width
            prev_height = height
            prev_filters = filters
            ...
            filters = prev_filters
            print('%5d %-6s       %d x %d / %d   %3d x %3d x%4d   ->   %3d x %3d x%4d' % (
                ind, 'gloavg', pool_size, pool_size, stride, prev_width, prev_height, prev_filters, width, height,
                filters))
            prev_width = width
            prev_height = height
            prev_filters = filters
...
def load_conv_bn(buf, start, conv_model, bn_model):
    ...
    start = start + num_b
    # fix shape not matching error
    conv_weights = torch.from_numpy(buf[start:start + num_w])
    conv_weights = conv_weights.view(conv_model.weight.data.shape)
