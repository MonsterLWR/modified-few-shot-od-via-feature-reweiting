...
def data_augmentation(img, shape, jitter, hue, saturation, exposure, flag=True):
    ...
    if flag:
        ...
        dx = (float(pleft) / ow) / sx  # equals pleft / swidth
        dy = (float(ptop) / oh) / sy
...
def fill_truth_detection(labpath, w, h, flip, dx, dy, sx, sy):
    ...
    if os.path.exists(labpath) and os.path.getsize(labpath):
        ...
        for i in range(boxes.shape[0]):
            ...
            y1 = boxes[i][2] - boxes[i][4] / 2
            x2 = boxes[i][1] + boxes[i][3] / 2
            y2 = boxes[i][2] + boxes[i][4] / 2
            ...
            if flip:
                boxes[i][1] = 0.999 - boxes[i][1]

            if boxes[i][3] < 0.001 or boxes[i][4] < 0.001:
                continue
            label[count] = boxes[i]
            count += 1
            if count >= cfg.max_boxes:
                break
...
def fill_truth_detection_meta(labpath, w, h, flip, dx, dy, sx, sy):
    ...
    if os.path.exists(labpath) and os.path.getsize(labpath):
        ...
        for i in range(boxes.shape[0]):
            ...
            label[ind][count_cls[ind]] = boxes[i]
            # note that target class id is index of base_ids
            label[ind][count_cls[ind]][0] = ind
            count_cls[ind] += 1
            if sum(count_cls) >= cfg.max_boxes:
                break
...
def load_data_with_label(imgpath, labpath, shape, jitter, hue, saturation, exposure, data_aug=True):
    # data augmentation
    # this load function do not filter out labels not in base_ids
    img = Image.open(imgpath).convert('RGB')
    img, flip, dx, dy, sx, sy = data_augmentation(img, shape, jitter, hue, saturation, exposure, flag=data_aug)
